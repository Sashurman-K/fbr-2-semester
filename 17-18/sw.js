...
self.addEventListener('fetch', event => {
    ...
    if (url.pathname.startsWith('/content/')) {
        ...
        return; // Важно: завершаем обработку для content/*
    }

    // Для всех остальных запросов (статика) – Cache First, затем сеть
    event.respondWith(
...
self.addEventListener('push', (event) => {
    ...
    if (data.reminderId) {
        options.actions = [
            { action: 'snooze', title: 'Отложить на 30 секунд' }
        ];
    }
    event.waitUntil(
...
self.addEventListener('notificationclick', (event) => {
    ...
    if (action === 'snooze') {
        ...
        console.log('Snooze для reminderId:', reminderId, 'Тип:', typeof reminderId);
